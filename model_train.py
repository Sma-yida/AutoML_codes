...
from utils.utils_model_train import model_train_func
...
def run(config_path, session_id=None):
    ...
    try:
        ...
        # 2. 写入启动指示器
        startup_file = config['output_path']['model_startup_path']
        with open(startup_file, 'w') as f:
            f.write('1')

        # 2.5 打印启动信息和当前时间
        log_message = f"[START] 【模型训练】启动于 {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        print(log_message)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='运行模型训练')
